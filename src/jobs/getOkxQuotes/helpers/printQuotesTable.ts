import { OkxQuotesResult } from './types';

export function printQuotesTable(result: OkxQuotesResult): void {
  if (!result.ok || !result.quote) {
    console.error(`\n❌ OKX ошибка: ${result.error}`);
    return;
  }

  const q = result.quote;

  console.log(`\n📊 OKX ticker  |  symbol: ${q.symbol}  |  latency: ${q.latencyMs} ms`);
  console.table([
