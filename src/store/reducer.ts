...
import {
  BotList10, BotListTestArbitrum, BotListTestOptimism
...
export const initialState: AppState = {
  stateVersion: 1,
  appVersion: '0.0.2',
  serverStartedAt: new Date().toISOString(), // ✅ Время старта сервера
  botsTypesList: [
  ...
  // botsRulesList: BotListTestOptimism,
  // botsRulesList: BotListTestArbitrum,
  botsRulesList: [],
  botsList: [],
  errorList: [],
  apis: ApiStabs,
...
export function reducer(state: AppState = initialState, action: Action): AppState {
  switch (action.type) {
    ...
    // ---- botList ----
    case 'BOTS_RULES_LIST/SET_ALL': {
      return bump({ ...state, botsRulesList: action.payload.botsRulesList });
    }
